...
const StoreListPage: React.FC = () => {
  ...
  const { data: pageData, isLoading, error } = useAxiosQueryWithParams(API_URLS.MASTER.STORE_API, pageNumber, rowsPerPage, sortingOrder, {});
  ...
  const deleteMutation = useAxiosMutation(
    async (data: any) => {
      ...
      if (Array.isArray(data)) {
        response = await AxiosService.getInstance().axiosInstance.post(`${API_URLS.MASTER.STORE_API}/bulkdelete`, { ids: data });
      } else {
        response = await AxiosService.getInstance().axiosInstance.delete(`${API_URLS.MASTER.STORE_API}/${data}`);
      }
  ...
  const deleteAllSelected = (selectedRows: any[]) => {
    const storeIds = selectedRows.map(row => row.id);
    if (window.confirm(`Are you sure you want to delete ${storeIds.length} stores?`)) {
      deleteMutation.mutate(storeIds);
    }
  };

  const deleteSelected = (selectedRow: any) => {
    if (window.confirm(`Are you sure you want to delete ${selectedRow.name} store?`)) {
  ...
  const actionsColumn: ColumnDef<any> = {
    ...
    cell: ({ row }) => (
      <span className="isolate inline-flex rounded-md shadow-sm text-xs gap-1">
        <Button variant="primary" icon={<EyeIcon className="h-4 w-3 rounded-full" />} onClick={() => navigate(`/secure/master/stores/${row.original.id}`)} />
        <Button variant="warning" icon={<PencilIcon className="h-4 w-3 rounded-full" />} onClick={() => navigate(`/secure/master/stores/${row.original.id}/edit`)} />
  ...
  return (
    <>
      <PageHeader
        ...
        actions={[
          { label: "Create Store", variant: "primary", className: "text-xs px-3 py-0", onClick: () => navigate("/secure/master/stores/create") },
