...
const MachineDetailPage: React.FC = () => {
  ...
  let { id } = useParams();
  const { data: pageData, isLoading, error } = useAxiosQuery(`${API_URLS.MASTER.MACHINE_API}/${id}`);
  ...
  return (
    <>
      <PageHeader
        ...
        actions={[
          { label: "Edit Machine", variant: "info", className: "text-xs px-3 py-0", onClick: () => navigate("/secure/master/machines/" + id + "/edit") },
          { label: "List Machines", variant: "primary", className: "text-xs px-3 py-0", onClick: () => navigate("/secure/master/machines") }
        ]}
        className="px-4"
      />
