...
const MachineListPage: React.FC = () => {
  ...
  const { data: pageData, isLoading, error } = useAxiosQueryWithParams(API_URLS.MASTER.MACHINE_API, pageNumber, rowsPerPage, sortingOrder, {});
  ...
  const deleteAllSelected = (selectedRows: any[]) => {
    const machineIds = selectedRows.map(row => row.id);
    if (window.confirm(`Are you sure you want to delete ${machineIds.length} machines?`)) {
      deleteMutation.mutate(machineIds);
    }
  };

  const deleteSelected = (selectedRow: any) => {
    if (window.confirm(`Are you sure you want to delete ${selectedRow.name} machine?`)) {
  ...
  const actionsColumn: ColumnDef<any> = {
    ...
    cell: ({ row }) => (
      <span className="isolate inline-flex rounded-md shadow-sm text-xs gap-1">
        <Button variant="primary" icon={<EyeIcon className="h-4 w-3 rounded-full" />} onClick={() => navigate(`/secure/master/machines/${row.original.id}`)} />
        <Button variant="warning" icon={<PencilIcon className="h-4 w-3 rounded-full" />} onClick={() => navigate(`/secure/master/machines/${row.original.id}/edit`)} />
  ...
  return (
    <>
      <PageHeader
        ...
        actions={[
          { label: "Create Machine", variant: "primary", className: "text-xs px-3 py-0", onClick: () => navigate("/secure/master/machines/create") },
