...
const requisitionSchema = z.object({
  ...
  order_no: z.string().nonempty("Requisition number is required"),
...
const RequisitionFormPage: React.FunctionComponent = () => {
  const { setShowNotification } = useNotification();
  const navigate = useNavigate();
  const [selectedStore, setSelectedStore] = useState<any|undefined>(undefined);

  const { id } = useParams();
  const isEditMode = Boolean(id);
  const methods = useEasyForm(requisitionSchema);
  ...
  const store_id = useWatch({ control: methods.control, name: "store.id" });
  ...
  const mutation = useAxiosMutation(
    async (data: any) => {
      let response = null
      if (isEditMode) {
        response = await AxiosService.getInstance().axiosInstance.put(`${API_URLS.MASTER.REQUISITION_API}/${id}`, data);
      } else {
        response = await AxiosService.getInstance().axiosInstance.post(`${API_URLS.MASTER.REQUISITION_API}`, data);
      }
      return response?.data;
    },
    (data) => {
      if (isEditMode) setShowNotification('Requisition updated successfully', 'success');
      else setShowNotification('Requisition created successfully', 'success');
      if (isEditMode) { navigate(`/secure/master/requisitions/${id}`) } else { navigate('/secure/master/requisitions') };
    },
    (errors) => {
  ...
  const handleSubmit = async (data: z.infer<typeof requisitionSchema>) => {
    mutation.mutate(data);
  };

  const resetFormHandler = () => {
    let preStore = selectedStore;
    if (preStore === undefined) {
      preStore = stores?.data?.[0]
    }
    let defaultValues = {
    ...
    setSelectedStore(preStore);
  };

  useEffect(() => {
    resetFormHandler();
  }, [formData, isEditMode, stores, products]);
  ...
  return (
    <>
      <PageHeader label={`${isEditMode ? "Update" : "Create"} Requisition`}
        breadcrumbs={[{ label: "Dashboard" }, { label: "Requisition" }, { label: `${isEditMode ? "Update" : "Create"} Requisition` }]}
        actions={[
          { label: "List Requisitions", variant: "secondary", onClick: () => navigate("/secure/master/requisitions") }
        ]}
        className="px-4"
      />
        ...
        <EasyForm methods={methods} onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="border-b border-gray-900/10 pb-12">
            <div className="mt-10 grid grid-cols-1 gap-x-6 gap-y-0 sm:grid-cols-12">
              ...
              <Input name="order_no" label="Requisition Number" placeholder="Please enter order number" className='sm:col-span-6' />
              
              <Select name="store.id" label="Store Name" placeholder="Please select store" options={stores?.data} labelKey='name' valueKey='id' className='sm:col-span-6' selectClassName="rounded-lg" />
              <Input name="department" label="Department" placeholder="Please enter department" className='sm:col-span-6' />
          ...
          <div className="mt-6 flex items-center justify-end gap-x-6">
            <Button type="submit" loading={isLoading || isSubmitting} loadingText={`${isEditMode ? "Updating" : "Creating"}...`} variant="primary" label={`${isEditMode ? "Update" : "Create"} Requisition`} className="flex w-full justify-center" />
