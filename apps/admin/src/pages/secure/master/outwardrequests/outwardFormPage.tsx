...
const OutwardRequestFormPage: React.FunctionComponent = () => {
  const { setShowNotification } = useNotification();
  const navigate = useNavigate();
  const { id } = useParams();
  const isEditMode = Boolean(id);
  const methods = useEasyForm(formSchema);
  ...
  const { data: formData, isLoading: isDataLoading, error } = isEditMode ? useAxiosQuery(`${API_URLS.MASTER.OUTWARD_REQUEST_API}/${id}`) : {data: null, isLoading: false, error: null};
  ...
  const mutation = useAxiosMutation(
    async (data: any) => {
      let response = null
      if (isEditMode) {
        response = await AxiosService.getInstance().axiosInstance.put(`${API_URLS.MASTER.OUTWARD_REQUEST_API}/${id}`, data);
      } else {
        response = await AxiosService.getInstance().axiosInstance.post(`${API_URLS.MASTER.OUTWARD_REQUEST_API}`, data);
      }
      return response?.data;
    },
    (data) => {
      if (isEditMode) setShowNotification('Outward request updated successfully', 'success');
      else setShowNotification('Outward request created successfully', 'success');
      if (isEditMode) { navigate(`/secure/master/outwardrequests/${id}`) } else { navigate('/secure/master/outwardrequests') };
    },
    (errors) => {
        ...
        <EasyForm methods={methods} onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="border-b border-gray-900/10 pb-12">
            <div className="mt-10 grid grid-cols-1 gap-x-6 gap-y-0 sm:grid-cols-12">
              ...
              <Input name="order_no" label="Request Number" placeholder="Please enter order number" className='sm:col-span-6' />
              
              <Select name="customer.id" label="Customer" placeholder="Please select customer" options={customers?.data} labelKey='name' valueKey='id' className='sm:col-span-6' selectClassName="rounded-lg" />
...
export default OutwardRequestFormPage;
