...
const StoreDetailPage: React.FC = () => {
  ...
  let { id } = useParams();
  const { data: pageData, isLoading, error } = useStoreDetail(id);
  ...
  return (
    <>
      <PageHeader
        ...
        actions={[
          { label: "Edit Store", variant: "info", className: "text-xs px-3 py-0", onClick: () => navigate("/secure/warehouse/stores/" + id + "/edit") },
          { label: "List Stores", variant: "primary", className: "text-xs px-3 py-0", onClick: () => navigate("/secure/warehouse/stores") }
        ]}
        className="px-4"
      />
