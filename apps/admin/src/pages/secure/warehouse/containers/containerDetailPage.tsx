...
import { useContainerDetail } from "@hooks/warehouse/containers/containersHooks";

const ContainerDetailPage: React.FC = () => {
  const navigate = useNavigate();
  let { id } = useParams();
  const { data: pageData, isLoading, error } = useContainerDetail(id);

  return (
    <>
      <PageHeader
        label={`Container Detail - ${pageData?.name}`}
        breadcrumbs={[{ label: "Dashboard" }, { label: "Containers" }]}
        actions={[
          { label: "Edit Container", variant: "info", className: "text-xs px-3 py-0", onClick: () => navigate("/secure/warehouse/containers/" + id + "/edit") },
          { label: "List Containers", variant: "primary", className: "text-xs px-3 py-0", onClick: () => navigate("/secure/warehouse/containers") }
        ]}
        className="px-4"
      />
