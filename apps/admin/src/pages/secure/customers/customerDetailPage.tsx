...
import { useCustomerDetail } from "@hooks/customers/customersHooks";

const CustomerDetailPage: React.FC = () => {
  const navigate = useNavigate();
  let { id } = useParams();
  const { data: pageData, isLoading, error } = useCustomerDetail(id);
  ...
  return (
    <>
      <PageHeader
        ...
        actions={[
          { label: "Edit Customer", variant: "info", className: "text-xs px-3 py-0", onClick: () => navigate("/secure/customers/" + id + "/edit") },
          { label: "List Customers", variant: "primary", className: "text-xs px-3 py-0", onClick: () => navigate("/secure/customers") }
        ]}
        className="px-4"
      />
